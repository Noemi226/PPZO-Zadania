...
3. Opis relacji między klasami
   ...
   *  Kolekcja (Kompozycja/Agregacja): Klasa `HotelManager` posiada kolekcje (listy) obiektów `Room`, `Guest` oraz `Reservation`.
   *  Agregacja: Klasa `Reservation` agreguje w sobie obiekty `Guest` oraz `Room` (przechowuje referencje do nich jako swoje właściwości). Rezerwacja nie może istnieć bez gościa i pokoju.
   *  Parametry metod: Metoda `BookRoom` w klasie `HotelManager` przyjmuje jako parametry obiekty `Guest` i `Room`, łącząc je w nową rezerwację.
